...
# - Generate a self-signed TLS certificate for providerd and docker-backend
...
tls_key_file: "/shared/tls/key.pem"'
        ...
        ;;
    v0.13)
        BOOTSTRAP_VERSION=v013-1
        BACKEND_URL=http://docker-backend:9001
        BACKEND_CA=
        BACKEND_TLS=
        VOLUME_MOUNT=
...
openssl req -x509 -newkey ec -pkeyopt ec_paramgen_curve:prime256v1 \
    ...
    -addext "subjectAltName=DNS:localhost,DNS:providerd,DNS:docker-backend,IP:127.0.0.1"
...
# Mandatory placement authority; the placement-init service initializes it once
# against the empty chain/backend fleet before providerd starts.
...
callback_db_path: "/data/callbacks.db"
diagnostics_db_path: "/data/diagnostics.db"
releases_db_path: "/data/releases.db"
...
echo "$BOOTSTRAP_VERSION" > /shared/fred-bootstrap-version
