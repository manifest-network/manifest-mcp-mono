...
SDK-created clients preserve observed native CheckTx acceptance when a blocking inclusion timer or transaction lookup fails: `details.sent: true`, validated `details.transactionHash`, and the original failure through a non-enumerable cause chain. `cosmosTx` and `executeTx` retain their operation context. These failures remain non-retryable `TX_FAILED`; structured cancellation keeps `OPERATION_CANCELLED`. No transaction confirmation, code or height is inferred. The hash comes from observed acceptance, not an exception's claimed `txId`. Errors before observed acceptance, invalid returned hashes, and custom broadcast implementations keep their existing behavior.
