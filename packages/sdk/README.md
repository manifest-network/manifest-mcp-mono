...
SDK-created clients snapshot the signed bytes and compare their local SHA-256 transaction hash with the native CheckTx response. After native acceptance, a returned hash of the wrong length or value rejects before any transaction lookup, retaining the local hash. Blocking inclusion timeouts and lookup failures preserve `details.sent: true`, this `details.transactionHash`, and the original failure through a non-enumerable cause chain. `cosmosTx` and `executeTx` retain their operation context. These failures remain non-retryable `TX_FAILED`; a structured cancellation thrown by a custom lookup keeps `OPERATION_CANCELLED`. No transaction confirmation, code or height is inferred. An exception's claimed `txId` supplies no evidence. Errors before observed acceptance (including undecodable CheckTx responses), custom broadcast implementations and SYNC-only calls keep their existing behavior. Actual caller cancellation remains prompt and conservatively marks submission but currently lacks the guard's known hash (ENG-952).
