...
> **Follow-up Linear ticket:** `agent-core troubleshootDeployment — provider-side diagnostics (deferred from ENG-129 PR 4)` — to be filed by the PR author at merge time. The draft body (retained outside the commit) captures the deferred provider-side design (parallel-gather composition over `appStatus` / `appDiagnostics` / `getAppLogs`), the two additive public-contract fields (`walletProvider?` + `tail?`), the two missing internals (`summarize-app-status`, `render-troubleshoot-report`), and a 5-step suggested follow-up scope.
...
3. **`leases-by-tenant` pagination cap.** Both verify paths page-1-only (hard-coded `limit: 100`). Tenants with >100 leases may have a verify miss the target lease (false-negative `not_found`). Documented limitation; follow-up only if it bites in production.
