...
/** Static command metadata is independent of executable handlers, avoiding registration cycles. */
interface ModuleCatalog {
  [module: string]: {
    description: string;
    subcommands: { name: string; description: string; args?: string }[];
  };
}
...
export function throwUnsupportedSubcommand(
  ...
  const registry: ModuleCatalog = type === 'query' ? QUERY_MODULES : TX_MODULES;
