...
`validateManifest(value, compatibility?)` and `buildManifestPreview(input, compatibility?)` default to v0.13. Preview results identify the applied rules in `validation.fred_compatibility`, for both valid and invalid candidates. PR240 mode adds its stricter Compose/Unicode reserved-label and user-syntax checks. Deploy/update validate with the resolved provider policy before mutation. Standalone MCP previews use a configured global mode, or v0.13 when using a provider map because no provider has been selected; passing that preview does not imply acceptance under PR240. Orchestrated deployment resolves the selected provider's policy before previewing and showing the confirmation plan. Generated schema artifacts stay pinned to PR240 for drift checks.
