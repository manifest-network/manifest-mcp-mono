import type { FredLeaseStatus } from '@manifest-network/manifest-mcp-core';
import {
  abortableSleep,
  LeaseState,
  logger,
...
// Lease-readiness POLICY: the wait loop, its provision-status classification, and the two typed
// errors it raises. Lifted out of `http/fred.ts` in ENG-725 — every line below is that code
// verbatim except `pollLeaseReadiness`'s signature (see its own comment).
//
// WHY IT IS NOT IN THE TRANSPORT. Of ~137 lines exactly ONE was I/O: a `getLeaseStatus` call. The
// rest is policy — chain-state probe, per-iteration token re-mint, `provision_status`
// classification, a consecutive-failure budget, Retry-After clamping, abortable sleep, and the
// construction of two typed errors. Because that one call sat INSIDE the transport module it was a
// module-local reference, and vitest cannot rewrite those: no `vi.mock('./http/fred.js')` could
// ever intercept it, so the real `getLeaseStatus` ran against the real wire from inside an
// otherwise-mocked module (ENG-716). Injecting it as a parameter deletes that edge rather than
// relocating it, and lets a test drive the real loop with scripted statuses.
//
// The in-repo precedent is `EventTransport` in `core/src/ctx.ts`, whose doc settles the same
// question for fred's OTHER provider transport: "`open()` establishes ONE connection —
// reconnection, backoff, liveness and poll-fallback live in the consumer (fred's lease-status
// driver), NOT here." Same rule, applied to the poll.
//
// `http/fred.ts` keeps the published positional `pollLeaseUntilReady` as a thin wrapper over this,
// and re-exports the names below so `./http/fred.js` stays a stable import path.
 ...
 *
 * A function type rather than a one-method interface, matching `pollLeaseUntilReady`'s own two
 * existing injected functions (`authToken`, `checkChainState`) and Biome's default
 * `useShorthandFunctionType`. It is a ROLE interface in Fowler's sense — shaped by this
 * consumer's single interaction, not by the supplier's surface.
 *
 * `token` is passed per call because ADR-036 tokens are deterministic: a reused one is a
 * replay-rejected duplicate, so the loop re-mints on every iteration.
 *
 * NOTE the caller composes `providerUrl`, `fetchFn` and `allowLoopback` INTO the reader. Those are
 * transport concerns and the loop has no opinion about them; keeping them out is what lets a test
 * drive the real policy with scripted statuses and no wire at all.
 */
...
/** What the loop is waiting ON, as opposed to how it waits ({@link PollOptions}). */
export interface PollLeaseInput {
  readonly leaseUuid: string;
  /** Re-minted per iteration — see {@link LeaseStatusReader}. */
  readonly mintToken: () => Promise<string>;
}

/**
 * Wait until a lease is CONFIRMED ready, or fail with a typed reason.
 *
 * The body below is `pollLeaseUntilReady`'s, unchanged. Only the signature differs: the
 * `providerUrl` / `fetchFn` / `allowLoopback` transport triple and the
 * `string | (() => Promise<string>)` auth union are gone, replaced by the injected {@link
 * LeaseStatusReader} and `input.mintToken`. `http/fred.ts` keeps the old positional signature as
 * a wrapper, so the published API is unchanged.
 */
export async function pollLeaseReadiness(
  read: LeaseStatusReader,
  input: PollLeaseInput,
  opts: PollOptions = {},
): Promise<FredLeaseStatus> {
  const { leaseUuid, mintToken } = input;
  const {
    intervalMs = 3_000,
  ...
  while (Date.now() < deadline) {
    ...
    try {
      status = await read({ token, signal: abortSignal });
    } catch (err) {
