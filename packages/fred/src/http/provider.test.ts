...
import { validateProviderUrl, checkedFetch, parseJsonResponse, ProviderApiError } from './provider.js';
...
describe('validateProviderUrl', () => {
  ...
  it('strips trailing slashes', () => {
    expect(validateProviderUrl('https://example.com///')).toBe('https://example.com');
  });
  ...
  it('rejects HTTP non-localhost', () => {
    expect(() => validateProviderUrl('http://example.com')).toThrow(ProviderApiError);
  });
...
describe('checkedFetch', () => {
  it('returns response for successful requests', async () => {
    ...
    const res = await checkedFetch('https://example.com', undefined, 5000, mockFetch);
  ...
  it('throws ProviderApiError for non-ok responses', async () => {
    ...
    await expect(checkedFetch('https://example.com', undefined, 5000, mockFetch))
...
describe('parseJsonResponse', () => {
  it('parses valid JSON', async () => {
    ...
    const result = await parseJsonResponse<{ status: string }>(res, 'https://example.com');
    expect(result.status).toBe('ok');
  });

  it('throws ProviderApiError for invalid JSON', async () => {
    const res = new Response('not json', { status: 200 });
    await expect(parseJsonResponse(res, 'https://example.com'))
      .rejects.toThrow(ProviderApiError);
  });
});
