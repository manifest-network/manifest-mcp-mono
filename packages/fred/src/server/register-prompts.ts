      ...
      messages: [
        {
          ...
          content: {
            ...
            text: [
              ...
              `2. \`app_diagnostics({ lease_uuid })\` — record provision_status, fail_count, and the failure attribution: reason, message, and next_step when present. (Older providers send last_error instead of reason/message — use it as the fallback.)`,
              `3. \`get_logs({ lease_uuid, tail: 200 })\` — capture the most recent logs.`,
              ``,
              `How to read \`reason\`:`,
              `- It is a machine-readable failure code from a set the provider may EXTEND. If you do not recognize the value, say so plainly and rely on \`message\` and the logs — do not guess a meaning from the name.`,
              `- A non-empty \`reason\` does NOT mean the app is broken right now: it records the LAST failure. In particular, a \`ready\` app with \`reason: UpdateFailed\` is RUNNING — its last update failed and was rolled back to the previous version. Never advise a redeploy for that case without first saying the app is currently up.`,
              `- Decide "is it broken" from \`provision_status\` and the chain state, never from the presence of \`reason\`.`,
              `- \`CleanupFailed\` and \`VolumeCleanupExhausted\` are provider-side and the user has no action available: say so and point to operator escalation rather than suggesting a retry.`,
              `- \`Internal\` is also not the user's fault, but it IS worth retrying — these are often transient. Retry once, and escalate only if it recurs.`,
              ``,
              `Then summarize:`,
