...
- **`packages/core`** -- Shared library. Cosmos logic, on-chain tool functions, server utilities, LCD/REST adapter (`lcd-adapter.ts`). No provider HTTP clients (those live in fred), but core does host the shared SSRF-guarded fetch factory `createGuardedFetch` (`internals/guarded-fetch.ts`; `undici` + `node:dns` behind dynamic imports so the static graph stays isomorphic) used by fred + agent-core. It is exposed via the Node-only `@manifest-network/manifest-mcp-core/guarded-fetch` subpath export (entry `src/guarded-fetch.ts`, gated behind the `node` exports condition), **not** the package barrel (`src/index.ts`): a bundler that walks the universal barrel would otherwise statically resolve the dynamic `import("undici")` target and drag `node:async_hooks` into the graph, breaking browser builds (`createGuardedFetch`/`isBlocked`/`BLOCKED_RANGES_*` were removed from the barrel in ENG-281). The **pure** IP classifier (`isBlocked`/`isIpLiteral`/`BLOCKED_RANGES_*`) was split out into `internals/ssrf-classify.ts` (`ipaddr.js` only, no node builtins) in ENG-490 and is exposed on the **universal** `@manifest-network/manifest-mcp-core/ssrf` subpath (browser-safe; also off the barrel to keep the low-level primitive out of the public surface) — this is what fred's browser-safe URL-string SSRF check imports; `createGuardedFetch` (undici) stays Node-only on `/guarded-fetch`. Core also hosts the faucet ops (`requestFaucet`/`requestFaucetCredit`/`fetchFaucetStatus` + types, pure `fetch` + `zod`), hoisted from the chain server in ENG-531 and exposed on the **universal** `@manifest-network/manifest-mcp-core/faucet` subpath (browser-safe; **off** the barrel so `export type *` consumers like the SDK root don't inherit the testnet-only surface — the SDK re-exposes it on its own `/faucet` subpath; `chain` re-exports it non-breaking). Core also exposes `buildGasFee` + the gas-default constants (`DEFAULT_GAS_MULTIPLIER`/`DEFAULT_MAX_GAS`) on the **universal** `@manifest-network/manifest-mcp-core/gas` subpath (browser-safe; **off** the barrel, mirroring `/faucet`/`/ssrf`) so a first-party sibling that broadcasts on its own — the cosmwasm converter — can enforce the `COSMOS_MAX_GAS` gas-limit ceiling instead of passing `'auto'` (ENG-556). Not an MCP server itself. Built with `platform: "neutral"` for browser compatibility.
...
- **`packages/fred`** -- MCP server with 12 provider/Fred tools: `browse_catalog`, `deploy_app`, `app_status`, `get_logs`, `restart_app`, `update_app`, `restore_app`, `app_diagnostics`, `app_releases`, `check_deployment_readiness`, `build_manifest_preview`, `wait_for_app_ready`. `restore_app` (ENG-599) recovers a CLOSED/credit-exhausted lease's retained volumes onto a fresh lease (saga: pre-flight retained-check → create-lease → POST /restore → cancel-lease on terminal failure). `app_status`/`app_diagnostics` also surface Fred retention metadata (`retained_until`/`items`/`restore_hint`) for CLOSED leases (ENG-600). `app_releases` omits the base64 deployment manifest Fred sends on every release (reporting only `manifest_bytes`) and caps history to the 20 most recent, with `release_count`/`truncated` — the blob and the count were both unbounded into model context (ENG-669). Contains HTTP clients (auth, provider, fred) and tool implementations. Also exports all tool functions and HTTP clients for library consumers. Stack manifests use `{ services: { ... } }` wrapper format; upload payloads are `Uint8Array`. `deploy_app` accepts optional `custom_domain` (FQDN to claim on the freshly-created lease) and `service_name` (required for stack leases). All provider/Fred HTTP is routed through core's SSRF-guarded fetch by default (gated `MANIFEST_FRED_FETCH_GUARDED`; the `FredMCPServer` constructor injects it via `server/fetch-gate.ts`) — provider URLs come from on-chain SKU records, so the guard blocks a malicious provider from pointing them at an internal host. ENG-490 adds a second, browser-safe SSRF layer sharing that same switch: `validateProviderUrl`/`isUrlSsrfSafe` (fred `http/provider.ts`, exported on the fred barrel + SDK `/deploy`) default-deny a provider `apiUrl` that is a literal private/internal/loopback/metadata IP (obfuscated encodings normalized via `new URL()`; fails open on DNS hostnames — connect-guard is authoritative there). Loopback is opt-in via `{ allowLoopback }`; the server threads `ctx.allowLoopback = !guarded` so it's allowed only when `MANIFEST_FRED_FETCH_GUARDED=0`.
- **`packages/cosmwasm`** -- MCP server with 2 converter tools: `get_mfx_to_pwr_rate`, `convert_mfx_to_pwr`. Requires `MANIFEST_CONVERTER_ADDRESS` env var. Uses the on-chain MFX→PWR converter contract (CosmWasm smart contract with `{"convert":{}}` execute and `{"config":{}}` query). `convert_mfx_to_pwr` broadcasts through `buildGasFee` (core's `/gas` subpath) so the `COSMOS_MAX_GAS` ceiling bounds it too — it does not pass `'auto'` (ENG-556).
...
- **`packages/sdk`** -- Aggregating, app-building SDK for Manifest + Fred (`@manifest-network/manifest-sdk`), composing `core` + `fred` + `agent-core` over manifestjs. Not an MCP server itself — it is the supported external app-building entry point (the reference consumer is Barney, the Manifest web frontend). Exposes an aggregating root barrel plus scoped, tree-shakable subpaths: `/reads`, `/catalog`, `/deploy`, `/chain`, `/faucet`, `/orchestration`, and a Node-only `/node` (the SSRF-guarded `createGuardedFetch`). Built with `platform: "neutral"`. Added in v0.15.0 (ENG-442); see `packages/sdk/README.md` and the cookbook `docs/library-usage.md`. The `/chain` subpath surfaces the generic tier-2 chain escape hatches `cosmosQuery`/`cosmosTx` (re-exported from core, not the chain server); `/faucet` surfaces the faucet ops (`requestFaucet`/`requestFaucetCredit`/`fetchFaucetStatus` + types) — hoisted into core's universal `/faucet` subpath and kept off both barrels so `export type *` can't leak them onto the SDK root (testnet/operator concern; ENG-446 M7, ENG-531).
...
4. **Orchestration (elicitation-driven)** -- the agent server's tools in `packages/agent` wrapping `packages/agent-core`'s `deployApp` / `manageDomain` / `troubleshootDeployment` / `closeLease`, plus a read-only `lookup_custom_domain_orchestrated` over `manageDomain`'s lookup path. The wrapper translates agent-core's typed callbacks into MCP `elicitation/create` requests + `notifications/progress` events; host surfaces drive the bidirectional flow over standard MCP wire.
...
- `ipaddr.js` is force-pinned to `2.4.0` tree-wide via the root `package.json` `overrides`. The SSRF guard (`core`'s `internals/guarded-fetch.ts` — moved there from agent-core in ENG-268 so `fred` can share it; re-exported from agent-core's Node-only `@manifest-network/manifest-agent-core/guarded-fetch` subpath, fenced off its package barrel in ENG-309 mirroring core's ENG-281 split) treats `ipaddr.js`'s `range()` as the sole source of truth (default-deny on any non-`'unicast'` label); proxy-addr (transitive via the MCP SDK → express) pulls `ipaddr.js@1.9.1`, whose stale RFC table misclassifies reserved ranges (e.g. `198.18.0.0/15`, `100::/64`) as `'unicast'` and would bypass the guard. Do not remove the override. (ENG-218)
- **Stale/worktree dep drift:** the root `overrides` + lockfile fix the *resolved* version, but the on-disk `node_modules` can still diverge from the lockfile. A freshly-created `git worktree` (under `.claude/worktrees/`) starts with **no** `node_modules` and resolves transitive deps *up* to the repo-root hoisted copy; a checkout that pulled a lockfile change without reinstalling keeps the stale copy. Either can silently load the wrong version of a pinned transitive dep (e.g. `ipaddr.js@1.9.1` instead of the pinned `2.4.0`), producing misleading SSRF-guard test results — false failures, or worse, false passes that hide a broken guard. Run `npm install` in a new worktree and after pulling lockfile changes. CI is unaffected — it uses `npm ci`, which installs exactly the lockfile. (ENG-220)
- **Internal sibling pinning + identity peers (ENG-463):** internal `@manifest-network/*` sibling deps are **exact-pinned** (no `^`/`~`) in `dependencies`/`devDependencies` — the lockstep-monorepo supply-chain-determinism convention (the `@solana/kit` idiom). The two identity-bearing shared packages — `core` (branded types + the `CosmosClientManager` singleton) and `fred` (`ProviderApiError`, public via `@manifest-network/manifest-sdk/deploy`) — are declared as **non-optional `peerDependencies` (caret) + exact `devDependencies`** on the non-terminal libraries that use them (`chain`/`lease`/`cosmwasm`/`fred`/`agent-core`/`agent`), while the terminal install targets `sdk` and `node` carry them as regular exact `dependencies` and thereby provide the single hoisted copy. This is the structural guard against the dual-package / `instanceof` / singleton hazard under version skew (the react-dom→react, typescript-eslint `eslint-plugin`→`parser` pattern). `scripts/version.mjs` enforces the ranges — internal deps/devDeps exact, internal peers caret (`^`) — and iterates only internal names, so `core`'s external `vitest` peer is left alone. npm does not support the `workspace:` protocol, so `version.mjs` is the npm-native equivalent of publish-time range replacement; re-run it at the current version to normalize after any dependency edit, and never hand-edit `package-lock.json`. It also normalizes `examples/*` workspace members (private, e.g. `sdk-acceptance`) — bumping their version + internal ranges so a release never needs a manual example bump — while the normalize set stays `packages/*` names only (ENG-476).
...
- The SDK's public API surface is guarded by `publint` + `@arethetypeswrong/core` (run via the tsdown build), NOT `@microsoft/api-extractor` — api-extractor's release-tag gate is deliberately not adopted (ENG-446 T1); `@public`/`@beta` JSDoc tags are informational. A TypeDoc reference site is a possible future follow-up.
- Brand casts use a two-family model: `parseX()` validates at trust boundaries; `asX()` is an UNCHECKED trusted-read cast. Enforced by convention + review only (Biome cannot restrict call sites by module) — do not use `asX` outside trusted-read/adapter code (ENG-446 M2).
- Faucet / gas funding is intentionally NOT on the SDK surface — it is a testnet/operator concern (the `chain` MCP server's `request_faucet` covers it). If in-app gas onboarding is ever needed, expose it as a separate testnet-scoped subpath, never the main barrel (ENG-446 M7).
- **SDK read-surface principle (ENG-537):** the SDK is the foundation — *sufficient, not exhaustive*. A consumer never needs a second manifestjs client: everything is reachable through **one** client. What the SDK *adds* — branding, ctx composition (rate limiting, abort, retry), not-found semantics, multi-call orchestration, transport lifecycle — it owns as a typed read. What manifestjs already defines (the 1:1 wire query) it **carries** via the `@public` `client.query` seam on the read client (a Telescope/cosmjs drop-down: `fromJSON`-converted → numeric enums, but it bypasses the rate limiter), **not** copies. A read earns a typed wrapper only when it adds at least one of those; a 1:1 passthrough that adds none (e.g. `creditAddress`, single-denom `bank.balance`, standalone `creditAccount`/`creditEstimate`) is reached through `client.query`, never re-wrapped — wrapping it would just reinvent manifestjs. Triage "the SDK is missing read X" against this rule before adding surface. `client.query` is documented in `packages/sdk/README.md` + `docs/library-usage.md`.
...
| `MANIFEST_FRED_FETCH_GUARDED` | No (fred server) | `1` (default ON; SSRF guard for provider HTTP; same parser as `MANIFEST_AGENT_FETCH_GUARDED`) |
