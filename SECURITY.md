...
- Provider/Fred HTTP is routed through an SSRF-guarded fetch (`createGuardedFetch`) that resolves the target at connect time and default-denies any non-`unicast` IP (loopback, link-local, private, reserved, etc.), so a malicious on-chain provider URL can't point a server at an internal host. It is on by default and gated per server by `MANIFEST_FRED_FETCH_GUARDED` / `MANIFEST_AGENT_FETCH_GUARDED`. Note this guard is scoped to the MCP servers and to the SDK's `createFredClientNode`; other library entry points on Node (the base `createFredClient`, the raw HTTP functions on `/deploy`) are **unguarded by default** and must inject a guarded fetch. The always-on URL-validation layer applies everywhere regardless (see [`docs/security.md`](docs/security.md)).
...
- Decrypted wallet material is held in memory. Bootstrap shuts down the transport and chain clients, then invokes wallet `disconnect()` on transport closure or SIGINT/SIGTERM, within a bounded grace period. Library consumers should release their wallets explicitly. Dropping references cannot deterministically zero immutable JavaScript strings; see [wallet handling](docs/security.md#wallet-handling).
