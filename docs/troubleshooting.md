...
| Maintenance | `UPDATE_INDETERMINATE`, `RESTART_INDETERMINATE`, `MAINTENANCE_REQUEST_FAILED`, `MAINTENANCE_WAIT_FAILED` | A maintenance POST has an uncertain outcome or a readiness wait failed. Preserve `details.idempotency_key`, lease, operation, and exact payload; a new key starts another command. Reconcile `app_status` / `app_releases`. Intentional retries reuse the original key with fresh authentication. An authentication refusal can be retried with fresh credentials and the same command key. Accepted-wait errors retain their original readiness/configuration diagnosis and command context. |
...
**Completed orchestrated recovery.** A successful `retry_set_domain` returns the normal deployment result. A completed `salvage_without_domain`, `cancel_lease`, or `close_lease` choice instead terminates the original deploy flow with `OPERATION_CANCELLED`, `details.lease_uuid`, and the selected `details.recovery_outcome`; this shape intentionally does not set `details.partial`. Salvage retains the live, billing lease. Cancel/close choices also report what teardown actually did in `details.stop_outcome` (`stopped`, `cancelled`, or `already_inactive`) and `details.lease_state`. `details.transaction_hash` is present exactly when `stop_outcome` is `stopped` or `cancelled`, and absent for `already_inactive` (including post-broadcast terminal reconciliation). After a failed blocking attempt converges to terminal state, `details.reconciliation` retains the failed attempt's actual machine diagnostics separately; its explicit `sent: true` also sets outer `sent: true`. The retained original teardown error is non-enumerable for SDK inspection. A terminal pre-query has no reconciliation snapshot, and missing submission metadata means unknown. Do not retry the deployment automatically. A teardown failure that does not converge still rejects.

For unresolved failures, if `deploy_app` creates the lease but cannot complete normally, `details.partial` is `true` and `details.lease_uuid` normally identifies the existing paid lease. Some lower-level partial failures also include `provider_uuid` / `provider_url` and use a `Deploy partially succeeded:` message prefix, but callers should branch on structured details instead of prose. **Where confirmation stopped decides what to do — the cases below call for different responses.**

If Fred declares partial success but omits the lease UUID or supplies a malformed one, agent-core cannot target recovery safely. It throws `TX_FAILED` with `details.partial === true`, no `details.lease_uuid`, and a bounded `details.rejected_lease_uuid` when Fred supplied a value. No recovery callback or side effect runs. Preserve this error for operator investigation and do not redeploy automatically: the unidentified lease may still exist and be billing.
