...
**Completed orchestrated recovery.** A successful `retry_set_domain` returns the normal deployment result. A completed `salvage_without_domain`, `cancel_lease`, or `close_lease` choice instead terminates the original deploy flow with `OPERATION_CANCELLED`, `details.lease_uuid`, and the selected `details.recovery_outcome`; this shape intentionally does not set `details.partial`. Salvage retains the live, billing lease. Cancel/close choices also report what teardown actually did in `details.stop_outcome` (`stopped`, `cancelled`, or `already_inactive`) and `details.lease_state`, with `details.transaction_hash` only when teardown broadcast a transaction. Do not retry the deployment automatically. A genuine teardown failure surfaces its original error instead.
