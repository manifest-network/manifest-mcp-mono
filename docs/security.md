...
| `createFredClient` or a raw HTTP fn with any explicit fetch (including `globalThis.fetch`) | **caller-defined** | treated as a deliberate opt-out and stays silent; injection alone does not prove the function is guarded |
...
| A Fred free function given a core client whose `fetch` option was omitted | **off** | core wraps the captured platform fetch with stable default-provenance metadata; Fred recognizes the tag and warns even if `globalThis.fetch` is later replaced |
...
**Defaulted unguarded Node paths warn.** `warnUnguardedOnce` in `packages/fred/src/http/unguarded-warning.ts` emits a single `logger.warn` — naming `createFredClientNode` and `createGuardedFetch` as the two fixes — from `createFredClient` (at construction) and from the shared `checkedFetch` / `fetchJsonChecked` transport (on the first HTTP call made without a custom `fetchFn`). The once-latch is **shared** by both, so a process that builds a client *and* calls raw functions still warns exactly once. Core preserves an omitted client option with a stable provenance tag on its required `ctx.fetch` wrapper; Fred reads that tag rather than comparing against the current global, so later reassignment of `globalThis.fetch` does not suppress the warning (ENG-672). A browser runtime and any explicitly injected function stay silent by design: injection is a deliberate opt-out, and this package cannot infer whether an arbitrary wrapper is guarded.
