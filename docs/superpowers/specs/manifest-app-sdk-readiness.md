...
| ☐ | Example app + acceptance test | 🔴 | none; **existing `e2e/docker-compose` + `deploy-roundtrip` harness unused by the spec** | **e2e against the live chain+provider** (deploy/query/connection/domain/restart/update/logs/batch/subscribe/stop, single+stack) — proves it *deploys*; **plus** a fail-closed browser build asserting no UNGUARDED node-only modules (`@cosmjs/crypto` guarded-crypto allowlisted) + size budget — proves it *bundles*. CI enforces the bundle/size/types subset; the LIVE deploy e2e is a local gate (no docker job in CI) · **P0** |
