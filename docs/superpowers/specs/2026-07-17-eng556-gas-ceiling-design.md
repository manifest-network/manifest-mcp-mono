...
Bonus: because the check runs **before** `calculateFee`, it also pre-empts a generic `new Uint53(gasLimit)` int53-range / non-finite throw inside `calculateFee` on an astronomically inflated or `NaN` estimate — we surface a clean `GAS_LIMIT_EXCEEDED` instead of an unclassified `Error`. (The related cosmjs [#1134](https://github.com/cosmos/cosmjs/issues/1134) *amount*-overflow is already handled by the installed fork via `Decimal`→string + a string-accepting `coins()`; the ceiling is about the *gas limit*, upstream of that.)
...
Fix: `cosmosTx` / `executeTx` **always resolve gas options** (on the non-explicit-fee path), and `buildGasFee` **always computes an explicit `StdFee`** instead of delegating `'auto'`. This is cosmjs's own documented pattern for bounding fees ([cosmjs#1134](https://github.com/cosmos/cosmjs/issues/1134): "manually simulate → compute `StdFee` → pass explicit fee instead of the auto flag"); the Telescope/Hyperweb "Calculating Fees" docs present manual `calculateFee` as a normal first-class pattern. We keep essentially the same `simulate × multiplier` + `calculateFee(gasLimit, gasPrice)` math cosmjs's `'auto'` runs — relocated so the ceiling can bite. One nuance (verified against the installed fork `@manifest-network/stargate@0.32.4-ll.3`, `signingstargateclient.js:176,197`): `buildGasFee` uses `Math.ceil` while cosmjs's `'auto'` uses `Math.round`, so the resolved default-path gas limit is at most **1 gas unit** higher — negligible, strictly safer (never under-provisions), and it unifies the default path with the override path (which already used `ceil`). Behavior-preserving up to that ≤1-gas rounding, plus the added abort.
...
- **`buildGasFee` (unit):** throws `GAS_LIMIT_EXCEEDED` with correct metadata when `ceil(simulate×mult) > maxGas`; returns `calculateFee(...)` when `≤`; `maxGas: -1` disables the clamp; a non-finite `gasLimit` (Infinity/NaN) throws when a ceiling is set; `options` absent still returns `'auto'`; the computed `StdFee` is `calculateFee(ceil(simulate × multiplier), gasPrice)` (buildGasFee's existing `ceil`).
