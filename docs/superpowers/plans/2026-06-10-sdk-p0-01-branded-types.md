...
> **⚠️ SUPERSEDED IN PART by spec v7 (big-picture re-review, 2026-06-10).** This plan was executed as written (8 brands), then the brand set was **scoped down to 5** — `TierName`, `Denom`, `ChainId` and their `parse*` constructors were removed (commit `bf9b841`): the Cosmos ecosystem uses bare `string` for those single-role ids, so branding them taxed every interop boundary for little safety. The **current** brand set is `Address`/`Tenant`, `LeaseUuid`/`ProviderUuid`/`SkuUuid`, `Fqdn` (constructors `parseAddress`/`parseLeaseUuid`/`parseProviderUuid`/`parseSkuUuid`/`parseFqdn`). Read the code blocks below as historical; `brands.ts`/`index.ts`/the brand tests are the source of truth.
