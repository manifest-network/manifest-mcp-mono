...
→ **P0 Plan 3 (canonical types chokepoint — has a design fork to resolve first):** relocate `fred`'s `DeployAppInput`/`ServiceConfig`/`BuildManifestOptions`/`ManifestFormat`/`ManifestValidationResult`/`DeployManifestInput`/`DeployAppResult` + the preview input + `FredLeaseStatus` into `core/src/manifest-types.ts`; unify `SkuSelector` → `SkuIntent` (branded `SkuUuid`/`ProviderUuid`/`TierName`); brand the result id-fields. **Decision needed:** `DeployAppResult` references `ConnectionDetails`/`LeaseState` and `DeployManifestInput` references `PollOptions` (today in `fred`'s HTTP layer) — relocating the value types to `core` requires either moving those supporting types to `core` too, or restructuring so the canonical types don't depend on `fred`-http types (and branding the id fields ripples a `parse*`/cast into `fred`'s `deployManifest` producer). Resolve this fork at the top of Plan 3.
