...
- [x] **Step 1: Tick the §H rows** — SDK package (A) ☑; example app + e2e acceptance ☑; browser build (no-UNGUARDED-node, fail-closed) ☑; per-subpath size budget + tree-shake symbol check ☑; publint/attw (A) ☑; dependency-cruiser incl. the example compose-only allowlist ☑. Note (a) the honest size-floor caveat (codegen-dominated; subpaths = an API-surface/symbol-scoping win; the symbol-absence check is the real tree-shake guard), and (b) **the CI-vs-local split (N-6): CI enforces the bundle/size/types/unit/depcruise subset; the LIVE e2e ("it deploys") is a local/manual gate (no docker job in ci.yml)** — do NOT claim "CI = done." — DONE: §H rows 103/104/108/109 ticked 🟢 with the size-floor caveat + the N-6 CI-vs-local split spelled out on the example-app row.

- [x] **Step 2: Full gate** — `npm run build`, `npm run lint`, `npx vitest run packages/`, `npm run check` (covers examples), `npm run depcruise` (+ fixtures + the example positive-control fire), the SDK browser-resolution + type-tripwire, the example browser build + the `/reads` symbol-absence + size budget; then the LIVE e2e (`docker compose up` → build → `npx vitest run e2e/sdk-acceptance.e2e.test.ts` single + stack → `down`). All green ⇒ **P0a is COMPLETE.** — DONE (STATIC gate, all green): build = 9 pkgs + example + attw/publint clean; lint exit 0; `vitest run packages/` = 116 files / 2151 passed, 17 skipped; `check` (packages+examples) clean; `depcruise` 356 modules clean + the fixtures + BOTH production-config positive controls fire (cast-guard.test.ts); SDK suite = 32 passed (browser-resolve + cast-guard + the `sdk.test-d.ts` type-tripwire); example suite = 15 passed (flow + the fail-closed browser-build + `/reads` symbol-absence); `size` = all 4 subpaths under budget. The **LIVE e2e was the B3 gate** (single-service variant = primary live gate); not re-run here — referenced.

- [x] **Step 3: Commit.** `git commit -m "docs(sdk): reconcile readiness scorecard — P0a acceptance complete (ENG-309)"`.
