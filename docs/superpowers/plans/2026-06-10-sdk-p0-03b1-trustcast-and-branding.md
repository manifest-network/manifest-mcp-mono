...
- [ ] **Step 4: Run → green.** `(cd packages/core && npm run build)`; `(cd packages/core && npm run lint)` exit 0; `npx vitest run packages/core packages/fred` → all pass. **Critical canaries — these three non-UUID-sentinel tests MUST stay green; they are the mechanical proof `as*` (not `parse*`) was used (a format-validating `parse*` would throw `INVALID_ARGUMENT` and redden them):**
