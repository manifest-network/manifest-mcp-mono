...
**HTTP 408 and 425 on reads.** Without a gRPC verdict, a `QUERY_FAILED` error with numeric `details.httpStatus: 408` permits retry, consistent with [RFC 9110 §15.5.9](https://www.rfc-editor.org/rfc/rfc9110.html#section-15.5.9). LCD reads use the existing `cosmosQuery` retry loop and acquire a rate-limit token per attempt; faucet `GET /status` requires an explicit caller retry loop, as shown below. Both respect that loop's configured budget, including `maxRetries: 0`. The allowance does not apply to other error categories or a status mentioned only in message text.
