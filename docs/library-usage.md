...
A receipt-bearing verification failure is terminal to `withRetry`, even when its query cause is transient. Use the returned hash and lease ID to inspect transaction and lease/domain state before deciding whether another mutation is needed; retry only the verification read when appropriate. Read-only `manageDomain` lookup is unchanged. A `closeLease` result of `already_inactive` supplies outcome/state but no transaction receipt, so verification errors add no inferred `sent` or hash. Normal retry classification still applies to preserved causes: retaining a formerly discarded abort or permanent transport cause can correctly make a no-receipt verification error terminal. That result may follow terminal-state reconciliation after a broadcast error; absence of a receipt does **not** establish that no broadcast was attempted. Successful public result shapes and callback interfaces are unchanged.
