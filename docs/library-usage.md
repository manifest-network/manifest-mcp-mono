...
Catch the orchestration's rejected promise to observe every failure. Query acquisition/execution failures and ordinary domain-mismatch or non-terminal/missing-lease results invoke `onFailure`; unexpected non-SDK errors elsewhere in post-mutation decoding or verifier spec/result validation become `QUERY_FAILED` without an additional failure callback. A close verifier success result missing its required state instead throws `TX_FAILED` without that callback. These failures still retain the mutation receipt and original cause through the outer wrapper.
