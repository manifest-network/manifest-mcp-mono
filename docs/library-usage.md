...
Client-manager connection catches preserve SDK identity only when code/message are strings and shallow details, including named consumer fields, can be read. If only unrelated detail inspection fails, normalization preserves the readable code/message, safely recovered machine facts, an existing cause whose property can be read, and the configured endpoint. Unreadable code/message, named fields, or the existing cause property during repair instead produce `RPC_CONNECTION_FAILED` with only the configured `url` (query connection) or `rpcUrl` (signing connection), `Error message unavailable`, and no cause. This also covers wallet signer acquisition before the connection retry loop. The checks do not recursively validate arbitrary nested diagnostic values or make state-changing custom accessors safe.

Cosmos query, fee estimation, build-context and transaction attribution guard diagnostic reads independently, retaining readable text and operation context. Diagnostic read failures during transaction attribution and malformed SDK code/message produce non-retryable `TX_FAILED`; this fallback preserves safe own-data submission/recovery fields, including a valid hash and independently validated receipt fields. Receipt facts are not inferred from a hash alone. Normalization retains an unreadable original as a non-enumerable cause. Readable attribution keeps existing spread semantics, and readable orchestration wrappers do not gain a cause, which would change their retry verdict.

Fred deploy diagnostics preserve independently readable terminal/readiness verdicts and known lease context. Restore's pre-POST token-failure and compensation formatting also guard unreadable messages. Later restore POST/poll diagnostics and terminal-error `withContext` are not fully guarded; ENG-996 tracks those remaining recovery boundaries. Agent deployment classification preserves independently readable partial-success evidence. MCP tools retain bounded, sanitized JSON responses and safely logged serialization-failure reasons. Fred resources reject with a fresh readable error, a bounded sanitized message (including plain-object and cross-realm messages), a readable numeric protocol code, and no arbitrary diagnostic data. Producer-side timeout/faucet/LCD and `executeTx` diagnostic inspection before retry remain separately tracked in ENG-983.
