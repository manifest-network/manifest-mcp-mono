...
Client-manager connection catches preserve SDK identity only when code/message are strings and shallow details can be read. Other initialization failures are normalized to `RPC_CONNECTION_FAILED` with the configured `url` (query connection) or `rpcUrl` (signing connection), without attaching a cause. Unreadable diagnostics use `Error message unavailable`. This also covers wallet signer acquisition before the connection retry loop. The checks do not recursively validate arbitrary nested diagnostic values or make state-changing custom accessors safe.

`cosmosTx` contains transaction-attribution inspection failures as non-retryable `TX_FAILED`, retaining module/subcommand/args and the original as a non-enumerable cause. Fred's post-lease handlers retain the known lease and recovery action even when error diagnostics cannot be read; agent deployment classification preserves independently readable partial-success evidence. MCP tool handling projects errors into the existing bounded, sanitized JSON envelope. Fred resources instead reject with a fresh readable error and a sanitized, bounded message, preserving a readable numeric protocol code and omitting arbitrary diagnostic data. Producer-side timeout/faucet/LCD inspection before retry remains separately tracked in ENG-983.
