...
REST clients verify `/cosmos/base/tendermint/v1beta1/node_info` against `config.chainId` before exposing queries. Signing clients verify the RPC chain ID before returning each new signing connection; when both endpoints are configured, both must match. Unavailable or malformed identity responses fail closed. The REST identity request uses `chainIdentityFetch` (the captured platform fetch by default), keeps a configured REST path prefix, rejects redirects, and permits at most 64 KiB within 10 seconds. This option configures only identity verification; generated LCD queries continue to use Axios and RPC connections use CosmJS. Provider HTTP uses the separate `fetch` option, so a trusted localhost/private chain endpoint does not require relaxing provider SSRF protection. These checks catch endpoint mistakes; a deliberately dishonest endpoint still requires a stronger trust model.
