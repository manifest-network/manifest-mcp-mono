...
**Failures after accepted submission.** SDK-created signing clients observe a validated 64-character hexadecimal hash returned by the native CheckTx broadcast. If its blocking inclusion timer or a later transaction lookup rejects, `cosmosTx` and `executeTx` preserve `details.sent: true`, `details.transactionHash`, operation attribution and a non-enumerable cause chain to the original thrown value. The error is non-retryable `TX_FAILED`; readable structured `OPERATION_CANCELLED` retains its cancellation code. No confirmation, transaction code or height is inferred. The observed hash wins over conflicting error metadata. A raw timeout, exception class or claimed `txId` alone supplies no evidence; pre-submission failures, invalid hashes and overridden broadcast implementations keep their existing behavior. Caller cancellation and SYNC results remain unchanged. Terminal teardown reconciliation can forward the sparse `{errorCode, sent, transactionHash}` snapshot, preserving retry protection without claiming inclusion.
