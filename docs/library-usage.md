...
Client-manager connection catches preserve SDK identity only when code/message are strings, shallow details (including named consumer fields) can be read, and retry inspection succeeds. A non-string Error message uses the fixed fallback instead of introducing retryable prose through coercion. If only unrelated detail inspection fails, normalization preserves the readable code/message/name, independently readable details, and an existing cause whose retry inspection succeeds. Configured `url`/`rpcUrl` fills missing endpoint context; supplied readable endpoint details retain precedence. If details, named fields, name or cause cannot be read, an independently non-retryable normalized verdict (such as cancellation or invalid configuration) survives without a cause. Otherwise the catch uses conservative `RPC_CONNECTION_FAILED`, endpoint-only details, `Error message unavailable`, and no cause. Unreadable code/message also use that fallback. This also governs connection failures surfaced by `createFredClient` and `createFredClientNode`, which delegate to the core factories; hostile SDK errors have no unconditional identity-preservation guarantee. It covers wallet signer acquisition before the connection retry loop. The checks do not recursively validate arbitrary nested diagnostic values or make state-changing custom accessors safe.
