...
3. `build_manifest_preview` — render and hash a deployment candidate. When `validation.valid` is true, `manifest_json` and `meta_hash_hex` exactly match the corresponding deploy; an invalid structured candidate is preserved only for diagnosis. **Run this before paying for a lease.** Catches bad env var names, wrong port format, malformed RFC 1123 service names, and a few dozen other rule violations.
